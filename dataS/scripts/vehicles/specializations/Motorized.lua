...
function Motorized.initSpecialization()
	g_configurationManager:addConfigurationType("motor", g_i18n:getText("configuration_motorSetup"), "motorized", nil, Motorized.getStoreAdditionalConfigData, nil, ConfigurationUtil.SELECTOR_MULTIOPTION, nil, , , 1)
...
function Motorized:onLoad(savegame)
	...
	if self.loadDashboardsFromXML ~= nil then
		...
		self:loadDashboardsFromXML(self.xmlFile, "vehicle.motorized.dashboards", {
			valueTypeToLoad = "directionForwardExclusive",
			valueObject = spec.motor,
			valueFunc = function (motor)
				return motor:getDrivingDirection() > 0
			end
