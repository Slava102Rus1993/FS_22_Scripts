...
function WinchControlRopeActivatable.new(vehicle, rope)
	...
	self.texts = vehicle[Winch.SPEC_TABLE_NAME].texts
	self.ropes = vehicle[Winch.SPEC_TABLE_NAME].ropes
	self.rope = rope
	self.activateText = ""
...
function WinchControlRopeActivatable:update(dt)
	if g_currentMission.player ~= nil then
		local x1, _, z1 = getWorldTranslation(g_currentMission.player.rootNode)
...
function WinchControlRopeActivatable:getIsActivatable()
	...
	if player ~= nil and player.baseInformation.currentHandtool == nil and player.isEntered then
