...
Winch = {
	...
	SPEC_NAME = g_currentModName .. ".winch",
	SPEC_TABLE_NAME = "spec_" .. g_currentModName .. ".winch",
	TREE_RAYCAST_DISTANCE = 5,
	CONTROL_RANGE = 5
}

source(g_currentModDirectory .. "scripts/specializations/activatables/WinchAttachTreeActivatable.lua")
source(g_currentModDirectory .. "scripts/specializations/activatables/WinchControlRopeActivatable.lua")
...
function Winch.initSpecialization()
	if g_iconGenerator == nil then
		g_configurationManager:addConfigurationType("winch", g_i18n:getText("configuration_winch"), "winch", nil, , , ConfigurationUtil.SELECTOR_MULTIOPTION)
	end

	g_storeManager:addSpecType("winchMaxMass", "shopListAttributeIconWinchMaxMass", Winch.loadSpecValueMaxMass, Winch.getSpecValueMaxMass, "vehicle")
...
function Winch.registerXMLPaths(schema, baseKey)
	...
	schema:register(XMLValueType.VECTOR_N, Cylindered.MOVING_TOOL_XML_KEY .. ".winch#ropeIndices", "List of rope indices which are update while moving part changes")
	schema:register(XMLValueType.VECTOR_N, Cylindered.MOVING_PART_XML_KEY .. ".winch#ropeIndices", "List of rope indices which are update while moving part changes")

	local schemaSavegame = Vehicle.xmlSchemaSavegame

	schemaSavegame:register(XMLValueType.INT, "vehicles.vehicle(?)." .. Winch.SPEC_NAME .. ".rope(?)#index", "Rope index")
	schemaSavegame:register(XMLValueType.VECTOR_TRANS, "vehicles.vehicle(?)." .. Winch.SPEC_NAME .. ".rope(?).attachedTree(?)#translation", "Translation of attached tree")
	schemaSavegame:register(XMLValueType.INT, "vehicles.vehicle(?)." .. Winch.SPEC_NAME .. ".rope(?).attachedTree(?)#splitShapePart1", "Split shape data part 1")
	schemaSavegame:register(XMLValueType.INT, "vehicles.vehicle(?)." .. Winch.SPEC_NAME .. ".rope(?).attachedTree(?)#splitShapePart2", "Split shape data part 2")
	schemaSavegame:register(XMLValueType.INT, "vehicles.vehicle(?)." .. Winch.SPEC_NAME .. ".rope(?).attachedTree(?)#splitShapePart3", "Split shape data part 3")
	ForestryPhysicsRope.registerSavegameXMLPaths(schemaSavegame, "vehicles.vehicle(?)." .. Winch.SPEC_NAME .. ".rope(?).attachedTree(?).physicsRope")
end
...
function Winch:onLoad(savegame)
	local spec = self[Winch.SPEC_TABLE_NAME]
	spec.texts = {}
	...
	self.xmlFile:iterate("vehicle.winch.rope", function (index, ropeKey)
		local rope = {}
	...
	self.xmlFile:iterate(configKey .. ".rope", function (index, ropeKey)
		local rope = {}
...
function Winch:onLoadFinished(savegame)
	if savegame ~= nil and not savegame.resetVehicles then
		local specKey = savegame.key .. "." .. Winch.SPEC_NAME
...
function Winch:onReadStream(streamId, connection)
	...
	for ropeIndex = 1, #spec.ropes do
		...
		if streamReadBool(streamId) then
			local numTrees = streamReadUIntN(streamId, rope.maxTreeBits)
			...
			for j = 1, numTrees do
				...
				elseif splitShapeId1 ~= 0 then
					table.insert(spec.splitShapesToAttach, {
						splitShapeId1 = splitShapeId1,
						splitShapeId2 = splitShapeId2,
						x = x,
						y = y,
						z = z,
						ropeIndex = ropeIndex
...
function Winch:onWriteStream(streamId, connection)
	...
	for i = 1, #spec.ropes do
		...
		if streamWriteBool(streamId, #rope.attachedTrees > 0) then
			streamWriteUIntN(streamId, #rope.attachedTrees, rope.maxTreeBits)
...
function Winch:onWriteUpdateStream(streamId, connection, dirtyMask)
	...
	if connection:getIsServer() then
		if streamWriteBool(streamId, bitAND(dirtyMask, spec.dirtyFlag) ~= 0) then
			for i = 1, #spec.ropes do
				streamWriteUIntN(streamId, MathUtil.sign(spec.ropes[i].controlInputSent) + 1, 2)
			end
		end
	else
		if streamWriteBool(streamId, bitAND(dirtyMask, spec.dirtyFlag) ~= 0) then
			for i = 1, #spec.ropes do
				streamWriteUIntN(streamId, MathUtil.sign(spec.ropes[i].controlDirection) + 1, 2)
			end
		end

		if streamWriteBool(streamId, bitAND(dirtyMask, spec.ropeDirtyFlag) ~= 0) then
			for i = 1, #spec.ropes do
				local rope = spec.ropes[i]
...
function Winch:onPostUpdate(dt, isActiveForInput, isActiveForInputIgnoreSelection, isSelected)
	...
	for i = 1, #spec.ropes do
		...
		if rope.isAttachModeActive then
			...
			if player ~= nil then
				if player.baseInformation.currentHandtool == nil and player.isEntered then
					...
					if not spec.treeRaycast.hasStarted then
						...
						spec.treeRaycast.startPos[3] = sz
						spec.treeRaycast.startPos[2] = sy
						spec.treeRaycast.startPos[1] = sx

						raycastClosest(x, y, z, dx, dy, dz, "onWinchTreeRaycastCallback", Winch.TREE_RAYCAST_DISTANCE, self, CollisionFlag.TREE, false, true)
					end
					...
					if rope.setupRope.physicsRopeIndex == nil then
						...
						if #rope.attachedTrees == 0 then
							...
							rope.setupRope:create(kinematicHelperNode, kinematicHelperNode, nil, , false)
						else
							local rootData = rope.attachedTrees[1]
...
function Winch:onRegisterActionEvents(isActiveForInput, isActiveForInputIgnoreSelection)
	if self.isClient then
		...
		if isActiveForInputIgnoreSelection then
			...
			if spec.controlGroupIndex ~= nil and spec.controlGroupIndex ~= self.spec_cylindered.currentControlGroupIndex then
...
function Winch:loadWinchRopeFromXML(xmlFile, key, rope)
	...
	xmlFile:iterate(key .. ".componentJoint", function (_, jointKey)
		local componentJoint = {
			index = xmlFile:getValue(jointKey .. "#jointIndex")
		}
...
function Winch:getIsWinchTreeAttachAllowed(ropeIndex, splitShapeId)
	...
	if rope ~= nil then
		...
		if rope.maxTreeMass < mass then
			return false, TreeAttachResponseEvent.TREE_ATTACH_FAIL_REASON_TOO_HEAVY
		end

		if rope.maxNumTrees <= #rope.attachedTrees then
...
function Winch:attachTreeToWinch(splitShapeId, x, y, z, ropeIndex, setupRopeData, noEventSend)
	local rope = self[Winch.SPEC_TABLE_NAME].ropes[ropeIndex]

	if rope ~= nil then
		local attachData = {
			activeHookData = rope.hookData:clone()
		}
		...
		if #rope.attachedTrees == 0 then
			...
			rope.mainRope:create(splitShapeId, attachData.activeHookData:getRopeTarget(), nil, , true, true)
		else
			local rootData = rope.attachedTrees[1]
		...
		if self.isClient and rope.samples.attachTree ~= nil then
	...
	for i = 1, #spec.ropes do
		if ropeIndex == nil or i == ropeIndex then
			...
			for ti = #rope.attachedTrees, 1, -1 do
				...
				if not self.isDeleting and ti == 1 and self.isClient then
					local x, y, z = attachData.activeHookData:getRopeTargetPosition()

					if rope.samples.detachTree ~= nil and not g_soundManager:getIsSamplePlaying(rope.samples.pullRope) then
...
function Winch:setWinchControlInput(ropeIndex, direction)
	...
	if not spec.isAttachable or self:getAttacherVehicle() ~= nil then
		...
		if rope ~= nil then
			if self.isServer then
				if direction ~= 0 and #rope.attachedTrees >= 1 then
					if direction ~= 0 then
...
function Winch:onWinchPlayerTriggerCallback(triggerId, otherId, onEnter, onLeave, onStay)
	...
	if (not spec.isAttachable or self:getAttacherVehicle() ~= nil) and (onEnter or onLeave) and g_currentMission.player ~= nil and otherId == g_currentMission.player.rootNode then
		for i = 1, #spec.ropes do
			local rope = spec.ropes[i]
...
function Winch:onWinchTreeRaycastCallback(hitObjectId, x, y, z, distance, nx, ny, nz, subShapeIndex, shapeId, isLast)
	...
	if hitObjectId ~= 0 and getHasClassId(hitObjectId, ClassIds.SHAPE) and getSplitType(hitObjectId) ~= 0 then
		if isLast then
			...
			if getRigidBodyType(shapeId) == RigidBodyType.STATIC and not spec.treeRaycast.isFirstAttachment then
				spec.treeRaycast.lastValidTree = nil
			...
			if centerX ~= nil then
				...
				if spec.treeRaycast.maxDistance < distanceToStart then
...
function Winch:updateExtraDependentParts(superFunc, part, dt)
	...
	if part.winchRopeIndices ~= nil then
		for i = 1, #part.winchRopeIndices do
			...
			local rope = self[Winch.SPEC_TABLE_NAME].ropes[index]
...
function Winch.loadSpecValueMaxLength(xmlFile, customEnvironment, baseDir)
	local maxLength = 0

	xmlFile:iterate("vehicle.winch.rope", function (_, ropeKey)
		maxLength = math.max(xmlFile:getValue(ropeKey .. "#maxLength", 0), maxLength)
	end)

	local lengthByConfig = {}
	...
	xmlFile:iterate("vehicle.winch.winchConfigurations.winchConfiguration", function (index, configKey)
		...
		xmlFile:iterate(configKey .. ".rope", function (_, ropeKey)
			maxConfigLength = math.max(xmlFile:getValue(ropeKey .. "#maxLength", 0), maxConfigLength)
		end)

		lengthByConfig[index] = maxConfigLength
	end)

	return {
		maxLength = maxLength,
		lengthByConfig = lengthByConfig
	}
end

function Winch.getSpecValueMaxLength(storeItem, realItem, configurations, saleItem, returnValues, returnRange)
	if storeItem.specs.winchMaxLength ~= nil then
		local maxLength = storeItem.specs.winchMaxLength.maxLength

		if configurations ~= nil then
			local configId = configurations.winch
			maxLength = storeItem.specs.winchMaxLength.lengthByConfig[configId] or maxLength
		else
			for _, length in pairs(storeItem.specs.winchMaxLength.lengthByConfig) do
...
function Winch.loadSpecValueMaxMass(xmlFile, customEnvironment, baseDir)
	...
	return {
		maxTreeMass = maxTreeMass,
		massByConfig = massByConfig
	}
end
...
ShopConfigScreen.registerCustomSpecValues = Utils.overwrittenFunction(ShopConfigScreen.registerCustomSpecValues, function (self, superFunc, values, storeItems, vehicle, saleItem)
	...
	for itemIndex, storeItem in ipairs(storeItems) do
		...
		if storeItem.specs ~= nil then
			if storeItem.specs.winchMaxLength ~= nil then
				maxWinchLength = math.max(Winch.getSpecValueMaxLength(storeItem, realItem, self.configurations, saleItem, true, false) or 0, maxWinchLength)
			end
	...
	if maxWinchMass ~= 0 then
		table.insert(values, {
			...
			value = string.format("%.1f%s", maxWinchMass, g_i18n:getText("unit_tonsShort"))
		})
	end
end)
