...
FellerBuncher = {
	...
	SPEC_NAME = g_currentModName .. ".fellerBuncher"
}

source(g_currentModDirectory .. "scripts/specializations/events/FellerBuncherCutEvent.lua")
source(g_currentModDirectory .. "scripts/specializations/events/FellerBuncherReleaseEvent.lua")
...
function FellerBuncher.initSpecialization()
	g_storeManager:addSpecType("fellerBuncherMaxTreeSize", "shopListAttributeIconMaxTreeSize", FellerBuncher.loadSpecValueMaxTreeSize, FellerBuncher.getSpecValueMaxTreeSize, "vehicle")
...
function FellerBuncher:onLoad(savegame)
	self.spec_fellerBuncher = self["spec_" .. FellerBuncher.SPEC_NAME]
	...
	spec.mainGrab = {
		animationName = self.xmlFile:getValue("vehicle.fellerBuncher.mainGrab#animationName"),
		speedScale = self.xmlFile:getValue("vehicle.fellerBuncher.mainGrab#speedScale", 1),
		releaseSpeedScale = self.xmlFile:getValue("vehicle.fellerBuncher.mainGrab#releaseSpeedScale", 1),
		componentJointIndices = self.xmlFile:getValue("vehicle.fellerBuncher.mainGrab#componentJointIndices", nil, true),
		dampingFactor = self.xmlFile:getValue("vehicle.fellerBuncher.mainGrab#dampingFactor", 20),
		jointIndex = 0,
		jointNode = nil,
		shapeId = nil,
		isUsed = false
	...
	spec.cutAnimation = {
		animationName = self.xmlFile:getValue("vehicle.fellerBuncher.cutAnimation#animationName"),
		speedScale = self.xmlFile:getValue("vehicle.fellerBuncher.cutAnimation#speedScale", 1)
	}
	...
	self.xmlFile:iterate("vehicle.fellerBuncher.treeSlot", function (index, key)
		local treeSlot = {
			animationName = self.xmlFile:getValue(key .. ".grab#animationName"),
			speedScale = self.xmlFile:getValue(key .. ".grab#speedScale", 1),
			releaseSpeedScale = self.xmlFile:getValue(key .. ".grab#releaseSpeedScale", 1)
		}
	...
	if self.isClient then
		...
		spec.samples = {
			cut = g_soundManager:loadSampleFromXML(self.xmlFile, "vehicle.fellerBuncher.sounds", "cut", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self),
			saw = g_soundManager:loadSampleFromXML(self.xmlFile, "vehicle.fellerBuncher.sounds", "saw", self.baseDirectory, self.components, 0, AudioGroup.VEHICLE, self.i3dMappings, self)
		}
	end

	spec.texts = {
		actionRelease = g_i18n:getText("action_releaseTrees", self.customEnvironment),
		actionCutTree = g_i18n:getText("action_woodHarvesterCut", self.customEnvironment),
		warningNoAccess = g_i18n:getText("warning_youAreNotAllowedToCutThisTree", self.customEnvironment),
		warningNoPermission = g_i18n:getText("shop_messageNoPermissionGeneral", self.customEnvironment),
		treeTooThick = g_i18n:getText("warning_treeTooThick", self.customEnvironment)
	}
...
function FellerBuncher:onUpdateTick(dt, isActiveForInput, isActiveForInputIgnoreSelection, isSelected)
				...
				if splitShapeId ~= 0 then
					...
					if isAllowed then
						...
						if spec.cutInputAction == nil then
							...
							if spec.cutDuration < spec.cutTimer then
								self:cutTree()
					...
					elseif isActiveForInputIgnoreSelection and warning ~= nil then
	...
	elseif isActiveForInputIgnoreSelection and spec.cutNode ~= nil then
		local cx, cy, cz = getWorldTranslation(spec.cutNode)
		local nx, ny, nz = localDirectionToWorld(spec.cutNode, 1, 0, 0)
		local yx, yy, yz = localDirectionToWorld(spec.cutNode, 0, 1, 0)
		local splitShapeId, minY, maxY, minZ, maxZ = findSplitShape(cx, cy, cz, nx, ny, nz, yx, yy, yz, spec.cutNodeSizeY, spec.cutNodeSizeZ)
		...
		if name == spec.mainGrab.animationName then
			...
			if spec.mountProcessTreeSlot == nil and spec.mainGrab.jointIndex == 0 then
				...
				if spec.mainGrab.jointIndex == 0 then
					...
					if self.isServer and spec.mainGrab.componentJointIndices ~= nil then
...
function FellerBuncher:getFellerBuncherCanCutSplitShape(splitShapeId, cx, cy, cz, nx, ny, nz, yx, yy, yz, minY, maxY, minZ, maxZ)
	...
	if WoodHarvester.getCanSplitShapeBeAccessed(self, cx, cz, splitShapeId) then
		...
		local lenBelow, lenAbove = getSplitShapePlaneExtents(splitShapeId, cx, cy, cz, nx, ny, nz, yx, yy, yz)
	...
	elseif not g_currentMission:getHasPlayerPermission("cutTrees", self:getOwner()) then
...
function FellerBuncher:cutTree(noEventSend)
	...
	if spec.foundSplitShape ~= nil then
		if self.isServer then
			...
			local splitType = g_splitTypeManager:getSplitTypeByIndex(getSplitType(spec.foundSplitShape))
	...
	if spec.cutInputAction ~= nil and self.isClient then
		g_soundManager:playSample(spec.samples.cut)
	end
...
function FellerBuncher:releaseMainArmTree()
	...
	if self.isServer and self.isServer and spec.mainGrab.componentJointIndices ~= nil then
		for j = 1, #spec.mainGrab.componentJointIndices do
			local componentJoint = self.componentJoints[spec.mainGrab.componentJointIndices[j]]
...
ShopConfigScreen.registerCustomSpecValues = Utils.overwrittenFunction(ShopConfigScreen.registerCustomSpecValues, function (self, superFunc, values, storeItems, vehicle, saleItem)
	...
	for itemIndex, storeItem in ipairs(storeItems) do
		...
		if storeItem.specs ~= nil and storeItem.specs.fellerBuncherMaxTreeSize ~= nil then
			local radius, _ = FellerBuncher.getSpecValueMaxTreeSize(storeItem, realItem, self.configurations, saleItem, true, false)
			maxRadius = math.max(radius or 0, maxRadius)
		end
	end

	if maxRadius ~= 0 then
		table.insert(values, {
			profile = "shopConfigAttributeIconMaxTreeSize",
			value = string.format("%d%s", MathUtil.round(maxRadius), g_i18n:getText("unit_cmShort"))
		})
	end
end)
